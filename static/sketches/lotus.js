/* Do not edit - autogenerated */
  ...
  var sketch = (p) => {
    ...
    p.setup = () => {
      ...
      const smaller = Math.min(height, width);
      span = smaller / 4.25;
    ...
    function Lotus(x, y, span2, petals, petalColor, innerColor, dotColor) {
      p.push();
      p.translate(x, y);
      SawTooth(span2, petals, petalColor, 2);
      const seg = Math.PI / petals;
      for (let i = 0; i < petals * 2; i++) {
        p.rotate(seg);
        p.fill(innerColor);
        p.circle(span2 / 4, 0, span2 / 12);
        p.triangle(span2 / 6, span2 / 16, span2 / 6, -span2 / 16, span2 / 3, 0);
        p.fill(dotColor);
        p.circle(span2 / 5, 0, span2 / 32);
      }
      p.noFill();
      p.strokeWeight(span2 / 48);
      p.stroke(petalColor);
      p.circle(0, 0, span2 / 3.1);
      p.pop();
    }
    function drawAll(span2, r) {
      let s1 = colors.next();
      let fg1 = colors.next();
      let fg2 = colors.next();
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (i % 2 === 0) {
          ...
          p.stroke(s1);
          p.strokeWeight(span2 / 32);
          p.line(span2 * 0.8, 0, span2 * 1.6, 0);
          p.noStroke();
        } else {
